...
func read(fd int, p []byte) (n int, err error) {
	var _p0 unsafe.Pointer
	if len(p) > 0 {
		_p0 = unsafe.Pointer(&p[0]) // 取切片首元素的地址
	} else {
		_p0 = unsafe.Pointer(&_zero) // 指向大段 空白内存？？
	}
...
// THIS FILE IS GENERATED BY THE COMMAND AT THE TOP; DO NOT EDIT
// 域名
// getdomainname, setdomainname - get/set NIS(Network Information Service) domain name
...
// epoll_create, epoll_create1 - open an epoll file descriptor
// #include <sys/epoll.h>
// int epoll_create(int size); // size 被忽略, 但是必须>0, 返回 epoll fd, 可以close(fd), 当所有指向此epoll实例的 fd 都closed, 内存会销毁相关资源
// int epoll_create1(int flags); // 如果flags是0, 和epoll_create 函数一样, 就只是size参数无作用而已, 只有 EPOLL_CLOEXEC 这一个标记, 支持 执行可执行文件时, 关闭此fd
// errno=ENFILE, 表示超过了资源限制
// errno=ENOMEM, 内核无足够内存可用
...
// THIS FILE IS GENERATED BY THE COMMAND AT THE TOP; DO NOT EDIT
// 等待信号, 触发信号处理函数的调用返回前 都sleep, 类似无超时的sleep
// 直到信号处理函数返回, pause 才会返回,
// 只会返回-1, errno=EINTR
...
// THIS FILE IS GENERATED BY THE COMMAND AT THE TOP; DO NOT EDIT
// 零拷贝发送, 一般用于 磁盘文件通过网络发出去，反过来不行，毕竟in_fd不能是socket，out_fd 可以是各种类型的文件
...
// addr是内存起始地址，作为建议，直接用NULL更好，length 必须>0
// 返回的指针才是真实地址
// fd所指文件的[offset, offset+length)被映射到内存， offset必须是页大小的整数倍
// prot -> protection 指定内存映射页的保护模式
// flags 确定更新的映射是否对映射相同区域的其他进程可见，以及是否更新将传送到基础文件。 以及其他
// 函数返回后fd可以理解close，映射依然有效
// int munmap(void *addr, size_t length);
// 返回值0表示成功，-1表示失败， errno会被设置
// 如果length和之前分配的不一样，只释放了部分空间，那么地址移动需要自己维护？？
