...
// TempFile creates a new temporary file in the directory dir, 创建临时文件
// opens the file for reading and writing, and returns the resulting *os.File.

// The filename is generated by taking pattern and adding a random
// string to the end. If pattern includes a "*", the random string
// replaces the last "*".  fmt.pattern replace with random string

// If dir is the empty string, TempFile uses the default directory for temporary files (see os.TempDir). 使用系统默认的临时目录

// Multiple programs calling TempFile simultaneously同时 will not choose the same file.
// The caller can use f.Name() to find the pathname of the file.

// It is the caller's responsibility to remove the file when no longer needed. 调用方负责删除文件
...
// TempDir creates a new temporary directory in the directory dir. 创建临时目录
